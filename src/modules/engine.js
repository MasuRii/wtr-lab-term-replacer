...
export async function performReplacements(targetElement) {
  ...
  if (state.settings.isDisabled || state.terms.length === 0) {
    log(state.globalSettings, `WTR Term Replacer: Skipping replacements - disabled: ${state.settings.isDisabled}, terms: ${state.terms.length}`);
    showProcessingIndicator(false);
    return;
  }

  try {
    log(state.globalSettings, `WTR Term Replacer: Found ${state.terms.length} terms to process, beginning replacement with retry mechanism`);
...
async function performReplacementsWithRetry(targetElement, maxRetries) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!validateElementStability(targetElement, elementStabilityCounter)) {
        log(state.globalSettings, `WTR Term Replacer: Element stability check failed, attempt ${elementStabilityCounter}`);
        ...
        if (chapterId) {
          ...
          if (!targetElement) {
            log(state.globalSettings, `WTR Term Replacer: Unable to re-acquire target element for chapter ${chapterId}`);
  ...
  // All retries failed
  log(state.globalSettings, `WTR Term Replacer: All replacement attempts failed, throwing error: ${lastError?.message || 'Unknown error'}`);
...
export async function executeReplacementLogic(targetElement) {
  ...
  if (textContent.length === 0) {
    ...
    log(state.globalSettings, `WTR Term Replacer: Target element has no text content in chapter ${chapterId} (${contentLength} chars), skipping`);
    return;
  }
