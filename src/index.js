  ...
  Handlers.handleDisableToggle = async function (e) {
    ...
    if (chapterBody) {
      try {
        ...
        log(state.globalSettings, `WTR Term Replacer: Error during ${shouldDisable ? "disable" : "enable"} operation:`, error);
...
async function main() {
  ...
  log(state.globalSettings, `WTR Term Replacer: Data loaded - terms: ${state.terms.length}, settings disabled: ${state.settings.isDisabled}`);
